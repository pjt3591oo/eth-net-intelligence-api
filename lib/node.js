...
Node.prototype.checkWeb3Connection = function()
{
	...
	if (!this._web3)
	{
		...
		else
		{
			...
			{
				// 해당 서버가 실행 후 이더리움 노드가 MAX_CONNECTION_ATTEMPTS 만큼 접속 실패할 경우
...
Node.prototype.setWatches = function()
{
	...
	this.updateInterval = setInterval( async function(){
		if(this.IS_NODE_GET_INFO) { self.getStats(); } 
		else {
			await notification('node에 문제가 발생했습니다. 노드상태 확인바랍니다.')
			process.exit(1);
		}			
	}, UPDATE_INTERVAL);
